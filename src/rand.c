...
#define arraycountof(x) (usizeof(x)/usizeof((x)[0]))
...
static int random_add_noise0 (const void *src_, unsigned len, int flags)
{
int retval;
	retval = 0;
const u8 *src;
...
struct timeval tv;
	...
	if (! (SEED_UPDATED & random_add_noise0 (&tv, usizeof(tv), DISABLE_STIR)))
		sha1_core (m_->seed, m_->noise);
...
void store_random (void *dst_, unsigned cb)
{
u8 *dst;
	dst = (u8 *)dst_;
	while (0 < cb) {
...
void random_add_noise (const void *src_, unsigned cb)
{
